...
void A_output(struct msg message)
{
  ...
  if ( windowcount < WINDOWSIZE) {
    ...
    // A needs to start the timer here for ACKS
    starttimer(A, 15);
...
void A_input(struct pkt packet)
{
  ...
  if (!IsCorrupted(packet)) {
    ...
    // Check if acknowlegment is same as next sequence 
...
void A_timerinterrupt(void)
{
  ...
  // restarting timer when timer resending 
  starttimer(A, 15);
  packets_resent++;
}       
...
void B_input(struct pkt packet)
{
  ...
  // next sequence correctly imputed using modulus of window size + 1 not 2
